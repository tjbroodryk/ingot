-- The caller's own handle for a memory — a conversation id, a job id.
--
-- What makes create idempotent. Two workers opening the memory for the same
-- conversation used to make two, and one of them became an orphan nobody
-- addressed; with this, the second create finds the first one's memory.
--
-- Unique per account, and only where set: most memories have no handle, and
