...
/**
 * An ingot is embedded with one model, and only one.
 *
 ...
 * number between -1 and 1 — it is just meaningless. So an ingot embedded with
...
function anIngot(): Ingot {
  return Ingot.cast({ accountId: 'acct_1', name: 'an ingot', now: new Date() });
}

describe('the vector space an ingot lives in', () => {
  it('is unclaimed until something is embedded', () => {
    // Most ingots never claim one: `"embed": true` is opt-in per column.
    expect(anIngot().embedding).toBeNull();
  });
