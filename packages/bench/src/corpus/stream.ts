...
export interface CorpusOptions {
   ...
   * - **Every adapter gets the identical array.** Drift happens here, once,
   *   upstream of every adapter, so it stays true that differences in the
   *   results are differences in what each one did with the same bytes.
   */
...
export function buildCorpus(
  ...
  // DRIFT: the first attempt at this listing was rate-limited.
  //
  // A payload on a records endpoint that carries no records, which is a thing
  // agents store all day and no schema anticipates. It is emitted *before* the
  // pages rather than instead of one, so it is a retry and not a hole: every
  // CI run still arrives below. What it tests is what `remember` does with a
  // body that has no `items`, no `ref` and no relation to the table the tool's
  // other results built — widen the table, invent a second one, or refuse.
  if (drift) {
    emit('ci.list_runs', { page: 1 }, [], { error: 'rate limited', retry_after: 30 });
  }
