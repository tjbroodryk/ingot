...
describe('the drifted corpus', () => {
  ...
  /**
   * The payload with no records in it. It is emitted as a retry rather than in
   * place of a page — the rule above is that nothing is lost — so it adds a
   * result without touching the run count of any record.
   */
  test('carries a rate-limit body that holds no records', () => {
    const bodies = drifted.filter(
      (result) => (result.result as Record<string, unknown>).error !== undefined,
    );
    expect(bodies).toHaveLength(1);
    expect((bodies[0] as (typeof bodies)[number]).refs).toHaveLength(0);
